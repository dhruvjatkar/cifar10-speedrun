...
# Attempt 125: Fully compiled resolvent with 5 CG iters for better accuracy.
# Based on 124's compiled single-batch structure (no bucket loop).
# 4 terms, 5 CG iters + warmstart, FP32.
...
def _zeropower_via_resolvents_compiled(
    ...
    """Fully compiled resolvent orthogonalizer — mirrors the NS path structure.
    Pads all grads to max_D×max_K, stacks, computes resolvent via batched CG.
    """
    if not filter_meta_data:
        return gradients_4d
    ...
    # Build all shifted matrices: (B*T, D, D)
    A_all = gram_scaled.unsqueeze(1) + shifts.view(1, -1, 1, 1) * eye.unsqueeze(0)
    A_all = A_all.reshape(B * num_terms, D, D)

    # Expand X for all terms: (B*T, D, K)
    X_exp = X.unsqueeze(1).expand(B, num_terms, D, K).reshape(B * num_terms, D, K)

    # Warm-start CG: Y0 = X / (σ + 1)
    ...
    # Weighted sum over shifts
    Y = Y.view(B, num_terms, D, K)
    orthogonalized = (Y * weights.view(1, num_terms, 1, 1)).sum(dim=1)
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        if group["orthogonalization_backend"] == "newtonschulz":
            ...
            # Single-batch compiled resolvent (no bucket loop)
            final_orthogonalized_grads = _zeropower_via_resolvents(
                nesterov_grads,
                filter_meta_for_current_step,
                self.max_D,
                self.max_K,
