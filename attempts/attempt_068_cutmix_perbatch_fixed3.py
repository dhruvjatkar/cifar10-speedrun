...
def main(run, model):
    ...
    # CutMix config: per-sample random position for fixed 3x3 patch, last 50%
    cutmix_size = 3
    cutmix_start_step = total_train_steps // 2

    # Compile the forward pass functions with reduced overhead
    @torch.compile(mode="max-autotune", fullgraph=True)
    def forward_step_soft(inputs, soft_labels, whiten_bias_grad):
        outputs = model(inputs, whiten_bias_grad=whiten_bias_grad)
        log_probs = F.log_softmax(outputs, dim=1)
        loss = -(soft_labels * log_probs).sum()
        return loss
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Apply per-sample fixed-patch CutMix in last 50% of training
            if step >= cutmix_start_step:
                B = inputs.size(0)
                rand_index = torch.randperm(B, device=inputs.device)
                # Each sample gets its own random 3x3 patch position
                cx = torch.randint(0, 32, (B,), device=inputs.device)
                cy = torch.randint(0, 32, (B,), device=inputs.device)
                x1 = (cx - cutmix_size // 2).clamp(0, 32 - cutmix_size)
                y1 = (cy - cutmix_size // 2).clamp(0, 32 - cutmix_size)
                # Apply per-sample patches
                for i in range(B):
                    ix1, iy1 = x1[i].item(), y1[i].item()
                    inputs[i, :, iy1:iy1+cutmix_size, ix1:ix1+cutmix_size] = \
                        inputs[rand_index[i], :, iy1:iy1+cutmix_size, ix1:ix1+cutmix_size]
                # Lambda is the same for all since patch size is fixed
                lam = 1.0 - (cutmix_size * cutmix_size) / (32 * 32)
                n_classes = 10
                smooth = 0.09
                soft_a = torch.full((B, n_classes), smooth / n_classes, device=inputs.device, dtype=inputs.dtype)
                soft_a.scatter_(1, labels.unsqueeze(1), 1.0 - smooth + smooth / n_classes)
                soft_b = torch.full((B, n_classes), smooth / n_classes, device=inputs.device, dtype=inputs.dtype)
                soft_b.scatter_(1, labels[rand_index].unsqueeze(1), 1.0 - smooth + smooth / n_classes)
                soft_labels = lam * soft_a + (1 - lam) * soft_b
                loss = forward_step_soft(inputs, soft_labels, whiten_bias_grad)
            else:
