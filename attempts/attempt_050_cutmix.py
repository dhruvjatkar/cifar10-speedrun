...
def main(run, model):
    ...
    # Forward step that supports cutmix via one-hot soft labels
    ...
    # Regular forward step (for when cutmix is not applied)
    @torch.compile(mode="max-autotune", fullgraph=True)
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            if torch.rand(1).item() < 0.5:
                ...
                soft_b.scatter_(1, labels_b.unsqueeze(1), 1.0 - smooth + smooth / n_classes)
