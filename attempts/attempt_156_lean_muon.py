...
# Attempt 156: Lean Muon optimizer with cached internals.
# Based on attempt 142 (persistent step tensors + max-autotune NS).
# Additional changes:
# 1. Cache filter_params, momentum_buffers, meta after first step (skip per-step loops)
# 2. Use set_to_none=False with foreach_zero_ for stable grad pointers
# 3. Remove explicit grad dtype casting (both are FP16)
# 4. Add TF32 flags for any FP32 intermediate ops
# 5. Inline SGD params into training loop (avoid optimizer1 overhead)
...
# Pre-allocate a persistent step tensor for CUDA graph pointer stability.
_step_tensor = torch.zeros(1, dtype=torch.float32, device="cuda")
_total_steps_tensor = torch.ones(1, dtype=torch.float32, device="cuda")
...
def _zeropower_via_newtonschulz5(
    ...
    # Read step info from persistent tensors (no recompilation!)
    progress_ratio = _step_tensor[0] / _total_steps_tensor[0]

    initial_target_mag = 0.5012
    final_target_mag = 0.0786
    target_magnitude = (
        initial_target_mag * (1.0 - progress_ratio) + final_target_mag * progress_ratio
    )
...
class Muon(torch.optim.Optimizer):
    def __init__(
        ...
        # Set the total steps tensor once
        _total_steps_tensor[0] = float(max(1, total_train_steps))
        # Cached state — populated after first step
        self._cache_ready = False
        self._cached_params = []
        self._cached_meta = []
        self._cached_momentum = []
    ...
    def step(self):
        ...
        if not self._cache_ready:
            # First step: populate caches
            for p_meta in self.filter_params_meta:
                p = p_meta["param"]
                if p.grad is not None:
                    self._cached_params.append(p)
                    state = self.state[p]
                    if "momentum_buffer" not in state:
                        state["momentum_buffer"] = torch.zeros_like(p.grad,
                            dtype=group["momentum_buffer_dtype"],
                            memory_format=torch.preserve_format)
                    self._cached_momentum.append(state["momentum_buffer"])
                    self._cached_meta.append((
                        p_meta["original_shape"],
                        p_meta["reshaped_dims"][0],
                        p_meta["reshaped_dims"][1],
                        len(self._cached_params) - 1
                    ))
            self._cache_ready = True

        filter_params_with_grad = self._cached_params
        momentum_buffers = self._cached_momentum
        filter_meta_for_current_step = self._cached_meta

        if not filter_params_with_grad:
            return

        # Get grads (stable pointers from set_to_none=False)
        grads = [p.grad for p in filter_params_with_grad]

        # Momentum: buf = momentum * buf + grad (no dtype casting needed, both FP16)
        torch._foreach_mul_(momentum_buffers, group["momentum"])
        torch._foreach_add_(momentum_buffers, grads)

        # Nesterov: ng = grad + momentum * buf
        if group["nesterov"]:
        ...
        # Update step tensor in-place for CUDA graph stability
        _step_tensor.fill_(float(self.step_count))
    ...
    def zero_grad(self, set_to_none: bool = True):
        if self._cache_ready:
            # Use stable in-place zeroing (avoids memory allocator overhead)
            grads = [p.grad for p in self._cached_params if p.grad is not None]
            if grads:
