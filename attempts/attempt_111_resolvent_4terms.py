...
# Attempt 111: Resolvent Muon with direct cholesky_solve + 4 quadrature terms.
# Based on attempt 110. Two speedups:
# (a) Direct solve (A+σI)Y=X instead of computing full inverse then multiplying.
# (b) 4 Gauss-Legendre terms instead of 8 (Kudo 2025 shows s≤4 suffices for FP32).
...
def _zeropower_via_resolvents(
    ...
    # Direct solve: (A+σI) Y = X  instead of computing full inverse
    ...
    # Expand X to match (B*num_terms, D, K)
    rhs = X.unsqueeze(1).expand(B, num_terms, D, K).reshape(B * num_terms, D, K)
    solved = torch.cholesky_solve(rhs, chol)  # (B*num_terms, D, K)
