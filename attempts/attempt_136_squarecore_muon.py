...
def _zeropower_via_squarecore5(
    ...
    X = torch.stack(grad_list)  # [B, D, K]

    # Keep the original normalization path unchanged.
    ...
    # Run the core path on a tall matrix so the square core is min(D, K) x min(D, K).
    transposed = False
    if X.size(1) <= X.size(2):
        X = X.transpose(1, 2).contiguous()  # [B, K, D]
        transposed = True

    x_dtype = X.dtype
    Xf = X.float()
    Y = torch.bmm(Xf.transpose(1, 2), Xf)  # [B, N, N], N = min(D, K)

    batch_size, core_dim, _ = Y.shape
    I = torch.eye(core_dim, device=Y.device, dtype=Y.dtype).unsqueeze(0).expand(
        batch_size, -1, -1
    )

    # Exact 3-step quintic NS, but executed through the square core.
    Q = I
    ...
    # Step 2
    YQ = torch.bmm(Y, Q)
    R = torch.bmm(Q.transpose(1, 2), YQ)
    R2 = torch.bmm(R, R)
    H = a * I + b * R + c * R2
    Q = torch.bmm(Q, H)
    ...
    # One rectangular apply at the end.
    X = torch.bmm(X, Q.to(x_dtype))
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        final_orthogonalized_grads = _zeropower_via_squarecore5(
            nesterov_grads,
            filter_meta_for_current_step,
            self.max_D,
            self.max_K,
