...
def hyperball_project_updates(grads, ref_norms_list):
    """Normalize update gradients to reference norm scale using vectorized ops."""
    g_norms = torch._foreach_norm(grads)
    scale_g = [R / (gn + 1e-7) for R, gn in zip(ref_norms_list, g_norms)]
    torch._foreach_mul_(grads, scale_g)
    return grads

def hyperball_project_weights(params, ref_norms_list):
    """Project weights back onto hypersphere using vectorized ops."""
    p_norms = torch._foreach_norm(params)
    scale_p = [R / (pn + 1e-7) for R, pn in zip(ref_norms_list, p_norms)]
    torch._foreach_mul_(params, scale_p)
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        # Hyperball: fused compiled update normalization
        ref_norms_aligned = [self.reference_norms[id(p)] for p in filter_params_with_grad]
        final_orthogonalized_grads = hyperball_project_updates(final_orthogonalized_grads, ref_norms_aligned)
        ...
        # Hyperball: fused compiled weight projection
        hyperball_project_weights(filter_params_with_grad, ref_norms_aligned)
...
def main(run, model):
    ...
    param_configs = [
        dict(params=[model.whiten.bias], lr=bias_lr, weight_decay=wd / bias_lr if bias_lr != 0 else 0.0),
        dict(params=norm_biases, lr=bias_lr, weight_decay=wd / bias_lr if bias_lr != 0 else 0.0),
        dict(params=[model.head.weight], lr=head_lr, weight_decay=wd / head_lr if head_lr != 0 else 0.0),
    ]
