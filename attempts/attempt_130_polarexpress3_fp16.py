...
# Attempt 130: Polar Express 3-iteration in FP16.
# Replaces Newton-Schulz 3-iter with Polar Express 3-iter (better-optimized
# per-stage coefficients), stays in FP16, uses baseline's single-batch padding.
# Drops the baseline's magnitude scaling (Polar Express just normalizes to unit norm).
...
# Polar Express coefficients (first 3 stages for fast convergence).
# These are optimized per-stage polynomials that converge faster than
# fixed Newton-Schulz coefficients.
_PE_COEFFS_RAW = (
    (8.28721201814563, -23.595886519098837, 17.300387312530933),
    (4.107059111542203, -2.9478499167379106, 0.5448431082926601),
    (3.9486908534822946, -2.908902115962949, 0.5518191394370137),
)
# Half-precision safety scaling (1.01 factor)
PE3_COEFFS = tuple(
    (a / 1.01, b / (1.01 ** 3), c / (1.01 ** 5)) for (a, b, c) in _PE_COEFFS_RAW
)

@torch.compile(fullgraph=True)
def _zeropower_via_polarexpress3(
    ...
    # Polar Express normalization: unit norm with safety factor
    X = X / (X.norm(dim=(1, 2), keepdim=True) * 1.01 + 1e-6)

    # Unrolled 3-stage Polar Express with per-stage coefficients
    a0, b0, c0 = PE3_COEFFS[0]
    A = X @ X.transpose(1, 2)
    B = b0 * A + c0 * (A @ A)
    X = a0 * X + B @ X

    a1, b1, c1 = PE3_COEFFS[1]
    A = X @ X.transpose(1, 2)
    B = b1 * A + c1 * (A @ A)
    X = a1 * X + B @ X

    a2, b2, c2 = PE3_COEFFS[2]
    A = X @ X.transpose(1, 2)
    B = b2 * A + c2 * (A @ A)
    X = a2 * X + B @ X
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        final_orthogonalized_grads = _zeropower_via_polarexpress3(
            nesterov_grads,
            filter_meta_for_current_step,
            self.max_D,
            self.max_K,
