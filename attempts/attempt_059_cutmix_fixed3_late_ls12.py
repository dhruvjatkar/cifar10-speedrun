...
def main(run, model):
    ...
    # CutMix config: fixed 3x3 patch, applied in last 50% of training
    cutmix_size = 3
    cutmix_start_step = total_train_steps // 2

    # Compile the forward pass functions
    ...
    def forward_step(inputs, labels, whiten_bias_grad):
        outputs = model(inputs, whiten_bias_grad=whiten_bias_grad)
        loss = F.cross_entropy(outputs, labels, label_smoothing=0.12, reduction="sum")
        return loss
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Apply fixed-patch CutMix in last 50% of training
