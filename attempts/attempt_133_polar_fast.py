...
def _legacy_fast_wide(X: torch.Tensor):
    return _poly_fast_wide_impl(X, LEGACY_COEFFS, norm_mul=1.0, eps=1e-6)
...
def _cans_fast_wide(X: torch.Tensor):
    return _poly_fast_wide_impl(X, CANS5_COEFFS, norm_mul=1.0, eps=1e-6)
...
def _polar_express_fast_wide(X: torch.Tensor):
    return _poly_fast_wide_impl(X, POLAR_EXPRESS_COEFFS, norm_mul=1.01, eps=1e-7)
...
def orthogonalize_batch(G: torch.Tensor, impl: str) -> torch.Tensor:
    ...
    if impl == "legacy_fast":
        ...
        X = _polar_express_fast_wide(X) if use_fast else _polar_express_direct(X)
    else:
