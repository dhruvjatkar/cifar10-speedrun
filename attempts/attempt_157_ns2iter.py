...
# Attempt 157: 2 NS iterations instead of 3 + lean Muon.
# Based on attempt 142 (persistent step tensors + max-autotune NS).
# Key radical change: reduce Newton-Schulz from 3 to 2 iterations.
# Quintic polynomial converges very fast — 2 iterations should give error < 1e-10.
# This saves ~33% of NS compute time (~20ms over 244 steps).
# Plus: cached optimizer state, TF32, no grad dtype casting.
...
def _zeropower_via_newtonschulz5(
    ...
    # Only 2 NS iterations (quintic converges fast enough)
    A = X @ X.transpose(1, 2)
    B = b * A + c * (A @ A)
    X = a * X + B @ X
