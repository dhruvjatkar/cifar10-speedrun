...
def _zeropower_via_squarecore_newtonschulz5_padded(
    ...
    """
    Exact 3-step NS5 written in square-core form, but kept as a single padded
    batch to preserve the original script's one-call execution model.

    This is intentionally different from the previous bucketed rewrite: on this
    CIFAR benchmark there are only a few conv filters, so splitting by shape
    hurts more than it helps on A100. The padded single-batch path keeps kernel
    launches low and lets batched GEMMs amortize better.
    """
    ...
    # Keep the square-core path in the input dtype here. On this benchmark,
    # half-precision Tensor Core throughput and lower launch overhead matter
    # more than the extra numerical headroom from a float32 core.
    ...
    # Q2 = (aI + bR + cR^2) Q1,  R = Q1 Y Q1^T
    R = torch.bmm(Q, Y)
    R = torch.bmm(R, Q.transpose(1, 2))
    RQ = torch.bmm(R, Q)
    RRQ = torch.bmm(R, RQ)
    Q = a * Q + b * RQ + c * RRQ

    # Q3 = (aI + bR + cR^2) Q2,  R = Q2 Y Q2^T
    R = torch.bmm(Q, Y)
    R = torch.bmm(R, Q.transpose(1, 2))
    RQ = torch.bmm(R, Q)
    RRQ = torch.bmm(R, RQ)
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        final_orthogonalized_grads = _zeropower_via_squarecore_newtonschulz5_padded(
            nesterov_grads,
            filter_meta_for_current_step,
            self.max_D,
            self.max_K,
    ...
    def tta(model, test_images) -> torch.Tensor:
        with torch.no_grad():
            ...
            for inputs_batch in test_images.split(B):
                inputs_batch = inputs_batch.contiguous(memory_format=torch.channels_last)
