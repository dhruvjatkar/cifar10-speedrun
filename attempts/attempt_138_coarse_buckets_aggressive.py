...
def _align_to_16(x: int) -> int:
    return ((max(1, x) + 15) // 16) * 16


def _ns5_bucket_cost(max_D: int, max_K: int, count: int) -> int:
    # Approximate 3-step NS5 cost for D <= K.
    # Per step: A=XX^T, A2=A@A, B@X  ->  4*D^2*K + 2*D^3 (up to a shared factor of 2)
    return count * (4 * max_D * max_D * max_K + 2 * max_D * max_D * max_D)


@torch.compile(fullgraph=True, mode="reduce-overhead")
def _zeropower_via_newtonschulz5_bucket_1(
    X: torch.Tensor,
    current_step: int,
    total_steps: int,
) -> torch.Tensor:
    ...
    X = X / (X.norm(dim=(1, 2), keepdim=True) + eps_stable)
...
@torch.compile(fullgraph=True, mode="reduce-overhead")
def _zeropower_via_newtonschulz5_bucket_2(
    X: torch.Tensor,
    current_step: int,
    total_steps: int,
) -> torch.Tensor:
...
@torch.compile(fullgraph=True, mode="reduce-overhead")
def _zeropower_via_newtonschulz5_bucket_3(
    X: torch.Tensor,
    current_step: int,
    total_steps: int,
) -> torch.Tensor:
...
class Muon(torch.optim.Optimizer):
    def __init__(
        ...
        defaults = dict(
            ...
            max_muon_buckets=max_muon_buckets,
        )
                    ...
                    self.filter_entries.append(
                        {
                            ...
                            "scale_base": float(len(p.data) ** 0.5),
                        }
                    )

        self.filter_params = [entry["param"] for entry in self.filter_entries]
        self.scale_base_by_param = {
            entry["param"]: entry["scale_base"] for entry in self.filter_entries
        }
        self.buckets = self._build_buckets(max_muon_buckets)
        self.bucket_workspaces = [
            torch.empty(
                (len(bucket["entries"]), bucket["max_D"], bucket["max_K"]),
                device=bucket["entries"][0]["param"].device,
                dtype=bucket["entries"][0]["param"].dtype,
            )
            for bucket in self.buckets
        ]

    def _build_buckets(self, max_muon_buckets: int):
        if not self.filter_entries:
            return []

        shape_groups = {}
        for entry in self.filter_entries:
            D, K = entry["reshaped_dims"]
            key = (D, _align_to_16(K))
            shape_groups.setdefault(key, []).append(entry)

        groups = []
        for (D, K), entries in shape_groups.items():
            groups.append({"D": D, "K": K, "entries": entries, "count": len(entries)})
        groups.sort(key=lambda g: (g["D"], g["K"]))

        n = len(groups)
        if n == 0:
            return []
        num_buckets = min(max_muon_buckets, n)

        seg_cost = [[0] * (n + 1) for _ in range(n)]
        seg_max = {}
        for i in range(n):
            max_D = 0
            max_K = 0
            count = 0
            for j in range(i, n):
                max_D = max(max_D, groups[j]["D"])
                max_K = max(max_K, groups[j]["K"])
                count += groups[j]["count"]
                seg_cost[i][j + 1] = _ns5_bucket_cost(max_D, max_K, count)
                seg_max[(i, j + 1)] = (max_D, max_K)

        inf = float("inf")
        dp = [[inf] * (n + 1) for _ in range(num_buckets + 1)]
        prev = [[-1] * (n + 1) for _ in range(num_buckets + 1)]
        dp[0][0] = 0.0

        for b in range(1, num_buckets + 1):
            for j in range(1, n + 1):
                for i in range(b - 1, j):
                    cand = dp[b - 1][i] + seg_cost[i][j]
                    if cand < dp[b][j]:
                        dp[b][j] = cand
                        prev[b][j] = i
        ...
        buckets = []
        for i, j in bounds:
            max_D, max_K = seg_max[(i, j)]
            bucket_entries = []
            for k in range(i, j):
                bucket_entries.extend(groups[k]["entries"])
            bucket_entries.sort(key=lambda e: e["reshaped_dims"])
            buckets.append(
                {
    ...
    def _select_ns_steps(self, do_norm_scaling: bool) -> int:
        progress = self.step_count / max(1, self.total_train_steps)
        if progress < 0.18:
            return 3
        if do_norm_scaling or (self.step_count % 8 == 0):
            return 3
        if progress < 0.72:
            return 2
        return 1
    ...
    def step(self):
        ...
        momentum_buffer_dtype = group.get("momentum_buffer_dtype", torch.half)
        ...
        if do_norm_scaling:
            ...
            active_params = [p for p in self.filter_params if p.grad is not None]
            if active_params:
                self.current_grad_norms = torch._foreach_norm(active_params)
                scale_factors = {
                    p: (self.scale_base_by_param[p] / (n + 1e-07)).to(p.dtype)
                    for p, n in zip(active_params, self.current_grad_norms)
                }
            else:
                scale_factors = {}
        ...
        for bucket, workspace in zip(self.buckets, self.bucket_workspaces):
            workspace.zero_()
            active_slots = []

            for slot, entry in enumerate(bucket["entries"]):
                p = entry["param"]
                g = p.grad
                if g is None:
                    continue
                ...
                mb.mul_(group["momentum"])
                if mb.dtype == g.dtype:
                    mb.add_(g)
                else:
                    mb.add_(g.to(mb.dtype))

                packed_view = workspace[slot, :D, :K]
                if group["nesterov"]:
                    packed_view.copy_(g.reshape(D, K))
                    packed_view.add_(mb.reshape(D, K), alpha=group["momentum"])
                else:
                    packed_view.copy_(mb.reshape(D, K))
                active_slots.append((slot, entry))

            if not active_slots:
                continue

            ns_steps = self._select_ns_steps(do_norm_scaling)
            if ns_steps == 3:
                orth = _zeropower_via_newtonschulz5_bucket_3(
                    workspace,
                    self.step_count,
                    self.total_train_steps,
                )
            elif ns_steps == 2:
                orth = _zeropower_via_newtonschulz5_bucket_2(
                    workspace,
                    self.step_count,
                    self.total_train_steps,
                )
            else:
                orth = _zeropower_via_newtonschulz5_bucket_1(
                    workspace,
                    self.step_count,
                    self.total_train_steps,
                )

            for slot, entry in active_slots:
                p = entry["param"]
                D, K = entry["reshaped_dims"]
                if do_norm_scaling and scale_factors is not None:
                    p.mul_(scale_factors[p])
                p.add_(
                    orth[slot, :D, :K].view(entry["original_shape"]),
                    alpha=-group["lr"],
                )
                if weight_decay_factor != 1.0:
                    p.mul_(weight_decay_factor)
...
def _get_loader_normalized_images(loader):
    if "norm" not in loader.proc_images:
        loader.proc_images["norm"] = loader.normalize(loader.images)
    return loader.proc_images["norm"]
...
def infer(model, loader, tta_level=0):
    ...
    test_images = _get_loader_normalized_images(loader)
    if tta_level < 2:
        model.eval()
...
def forward_step(model, inputs, labels, whiten_bias_grad):
...
def main(run, model):
    ...
    lr_factors2_initial = [
        group["initial_lr"] for group in optimizer1.param_groups[1:] + optimizer2.param_groups
    ]
