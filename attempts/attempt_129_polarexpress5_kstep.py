...
# Attempt 129: Bucketed Polar Express Muon with k-step orthogonalization skipping.
# Skips the expensive orthogonalization every other step (ortho_freq=2),
# using raw Nesterov direction on skip steps, to save time.
...
#############################################
# Muon optimizer (bucketed PolarExp + k-step)
#############################################
...
# Half-precision safety scaling from the Polar Express reference implementation:
# apply the 1.01 safety factor to all but the final asymptotic polynomial.
...
def _zeropower_via_bucketed_polarexpress5(
    gradients_4d: list[torch.Tensor],
    reshaped_D: int,
    reshaped_K: int,
) -> list[torch.Tensor]:
    # All tensors in this bucket share the exact same original shape.
    out_dtype = gradients_4d[0].dtype
    original_shape = gradients_4d[0].shape

    X = torch.stack(gradients_4d).reshape(len(gradients_4d), reshaped_D, reshaped_K)
    ...
    X = X / (X.norm(dim=(1, 2), keepdim=True) * 1.01 + 1e-6)

    hs = POLAR_EXPRESS_COEFFS[:5]
    for a, b, c in hs:
        A = X @ X.transpose(1, 2)
        B = b * A + c * (A @ A)
        X = a * X + B @ X
    ...
    X = X.reshape(len(gradients_4d), *original_shape).to(out_dtype)
...
class Muon(torch.optim.Optimizer):
    def __init__(
        ...
        self.last_ortho_step = 0
        self.total_train_steps = total_train_steps
        self.filter_params_meta = []
        self.cached_param_scale_factors = None
        ...
        for group in self.param_groups:
            for p in group["params"]:
                if len(p.shape) == 4 and p.requires_grad:
                    ...
                    bucket_key = (tuple(p.data.shape), reshaped_D, reshaped_K)
    ...
    def step(self):
        ...
        for p_meta in self.filter_params_meta:
            ...
            if p.grad is not None:
                ...
                filter_bucket_indices[p_meta["bucket_key"]].append(list_idx)

        if not filter_params_with_grad:
            return

        grads = [p.grad for p in filter_params_with_grad]

        if momentum_buffers is not None:
            torch._foreach_mul_(momentum_buffers, group["momentum"])
            grad_casts = [g.to(mb.dtype) for g, mb in zip(grads, momentum_buffers)]
            torch._foreach_add_(momentum_buffers, grad_casts)
        else:
            momentum_buffers = grads

        if group["nesterov"]:
            nesterov_grads = torch._foreach_add(
                grads, momentum_buffers, alpha=group["momentum"]
            )
        else:
        ...
        if do_norm_scaling:
            ...
            param_norms = torch._foreach_norm(filter_params_with_grad)
            # Keep the original script's behavior exactly here: periodically
            # rescale the PARAMETERS themselves, not the orthogonalized updates.
            self.cached_param_scale_factors = [
                (len(p.data) ** 0.5 / (n + 1e-7)).to(p.data.dtype)
                for p, n in zip(filter_params_with_grad, param_norms)
            ]

        do_orthogonalize = (
            self.step_count == 1
            or self.step_count - self.last_ortho_step >= group["ortho_freq"]
        )

        if do_orthogonalize:
            self.last_ortho_step = self.step_count
            final_update_grads = [None] * len(nesterov_grads)
            for bucket_key, bucket_indices in filter_bucket_indices.items():
                _, reshaped_D, reshaped_K = bucket_key
                bucket_grads = [nesterov_grads[i] for i in bucket_indices]
                bucket_orthogonalized = _zeropower_via_bucketed_polarexpress5(
                    bucket_grads,
                    reshaped_D,
                    reshaped_K,
                )
                for local_i, global_i in enumerate(bucket_indices):
                    final_update_grads[global_i] = bucket_orthogonalized[local_i]
        else:
            # Cheap refresh step: skip orthogonalization and use the current
            # Nesterov direction directly.
            final_update_grads = nesterov_grads

        if do_norm_scaling:
            torch._foreach_mul_(filter_params_with_grad, self.cached_param_scale_factors)
