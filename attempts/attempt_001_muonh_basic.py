...
class Muon(torch.optim.Optimizer):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                if len(p.shape) == 4 and p.requires_grad:
                    self.reference_norms[p] = p.data.norm().item()
    ...
    def step(self):
        ...
        # Hyperball: normalize updates
        for i, p in enumerate(filter_params_with_grad):
            R = self.reference_norms[p]
            g = final_orthogonalized_grads[i]
            g_norm = g.norm()
            if g_norm > 0:
                final_orthogonalized_grads[i] = g * (R / (g_norm + 1e-7))
        ...
        # Hyperball: project weights onto hypersphere
        for p in filter_params_with_grad:
            R = self.reference_norms[p]
            p_norm = p.data.norm()
            if p_norm > 0:
                p.data.mul_(R / (p_norm + 1e-7))
