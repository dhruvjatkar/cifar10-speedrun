...
def main(run, model):
    ...
    # CutMix config: progressive patch size (1->5), last 50% of training
    cutmix_start_step = total_train_steps // 2
    cutmix_end_step = total_train_steps
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Apply progressive CutMix in last 50% of training
            if step >= cutmix_start_step:
                # Progressive patch size: 1->5 over the CutMix window
                progress = (step - cutmix_start_step) / max(1, cutmix_end_step - cutmix_start_step)
                cutmix_size = 1 + int(4 * progress)  # 1, 2, 3, 4, or 5
                cx = torch.randint(0, 32, (1,)).item()
                cy = torch.randint(0, 32, (1,)).item()
                x1 = max(0, cx - cutmix_size // 2)
                y1 = max(0, cy - cutmix_size // 2)
                x2 = min(32, x1 + cutmix_size)
                y2 = min(32, y1 + cutmix_size)
                rand_index = torch.randperm(inputs.size(0), device=inputs.device)
                inputs[:, :, y1:y2, x1:x2] = inputs[rand_index, :, y1:y2, x1:x2]
                lam = 1.0 - (x2 - x1) * (y2 - y1) / (32 * 32)
                n_classes = 10
                smooth = 0.09
                soft_a = torch.full((inputs.size(0), n_classes), smooth / n_classes, device=inputs.device, dtype=inputs.dtype)
                soft_a.scatter_(1, labels.unsqueeze(1), 1.0 - smooth + smooth / n_classes)
                soft_b = torch.full((inputs.size(0), n_classes), smooth / n_classes, device=inputs.device, dtype=inputs.dtype)
