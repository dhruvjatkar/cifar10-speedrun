...
def _zeropower_via_newtonschulz5(
    ...
    # CANS per-step optimal coefficients (composition_4_ord_5, steps 1-3)
    # Step 1: aggressive inflation of small singular values
    A = X @ X.transpose(1, 2)
    B = -24.910491192120688 * A + 18.472094206318726 * (A @ A)
    X = 8.420293602126344 * X + B @ X

    # Step 2: refinement
    A = X @ X.transpose(1, 2)
    B = -3.051855546794681 * A + 0.574124102530270 * (A @ A)
    X = 4.101228661246281 * X + B @ X

    # Step 3: final refinement
    A = X @ X.transpose(1, 2)
    B = -2.753965023071620 * A + 0.540190278110893 * (A @ A)
    X = 3.680981925110916 * X + B @ X
