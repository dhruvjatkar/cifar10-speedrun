...
def _fused_scale_update_decay(
    ...
    """Fused: scale params by norm factors, apply update, apply weight decay."""
    for p, s, g in zip(params, scale_factors, grads):
        p.mul_(s)
        p.add_(g, alpha=-lr)
        p.mul_(wd_factor)

@torch.compile(fullgraph=True)
def _fused_update_decay(
    params: list[torch.Tensor],
    grads: list[torch.Tensor],
    lr: float,
    wd_factor: float,
) -> None:
    """Fused: apply update and weight decay."""
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        # NEW: Fused update step
        weight_decay_factor = 1 - group["lr"] * group["weight_decay"]
        if do_norm_scaling:
            _fused_scale_update_decay(
                filter_params_with_grad, scale_factors,
                final_orthogonalized_grads, group["lr"], weight_decay_factor)
        else:
            _fused_update_decay(
                filter_params_with_grad, final_orthogonalized_grads,
                group["lr"], weight_decay_factor)
