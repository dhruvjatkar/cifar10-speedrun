...
def _ns_common_pre(gradients_4d, filter_meta_data, max_D, max_K, current_step, total_steps):
    ...
    if not filter_meta_data:
        return gradients_4d, None, None, None
    grad_list = []
    for meta in filter_meta_data:
        ...
        g_padded = F.pad(g_reshaped, (0, max_K - reshaped_K, 0, max_D - reshaped_D), "constant", 0)
    ...
    if X.size(1) > X.size(2):
        X = X.transpose(1, 2)
        transposed = True
    return X, transposed, a, b, c

def _ns_common_post(X, transposed, gradients_4d, filter_meta_data):
    if transposed:
        X = X.transpose(1, 2)
    final = [None] * len(gradients_4d)
    for i, meta in enumerate(filter_meta_data):
        original_shape, reshaped_D, reshaped_K, list_idx = meta
        final[list_idx] = X[i, :reshaped_D, :reshaped_K].view(original_shape)
    return final

@torch.compile(fullgraph=True)
def _zeropower_ns_3step(gradients_4d, filter_meta_data, max_D, max_K, current_step, total_steps):
    X, transposed, a, b, c = _ns_common_pre(gradients_4d, filter_meta_data, max_D, max_K, current_step, total_steps)
    if X is None:
        return gradients_4d
    A = X @ X.transpose(1, 2); X = a * X + (b * A + c * (A @ A)) @ X
    A = X @ X.transpose(1, 2); X = a * X + (b * A + c * (A @ A)) @ X
    A = X @ X.transpose(1, 2); X = a * X + (b * A + c * (A @ A)) @ X
    return _ns_common_post(X, transposed, gradients_4d, filter_meta_data)

@torch.compile(fullgraph=True)
def _zeropower_ns_2step(gradients_4d, filter_meta_data, max_D, max_K, current_step, total_steps):
    X, transposed, a, b, c = _ns_common_pre(gradients_4d, filter_meta_data, max_D, max_K, current_step, total_steps)
    if X is None:
        return gradients_4d
    A = X @ X.transpose(1, 2); X = a * X + (b * A + c * (A @ A)) @ X
...
@torch.compile(fullgraph=True)
def _zeropower_ns_1step(gradients_4d, filter_meta_data, max_D, max_K, current_step, total_steps):
    X, transposed, a, b, c = _ns_common_pre(gradients_4d, filter_meta_data, max_D, max_K, current_step, total_steps)
    if X is None:
        return gradients_4d
    A = X @ X.transpose(1, 2); X = a * X + (b * A + c * (A @ A)) @ X
...
class Muon(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        # Conservative adaptive NS: 3 steps first 30%, 2 steps rest
        progress = self.step_count / max(1, self.total_train_steps)
        if progress < 0.30:
            ns_fn = _zeropower_ns_3step
        else:
            ns_fn = _zeropower_ns_2step

        final_orthogonalized_grads = ns_fn(
            nesterov_grads,
            filter_meta_for_current_step,
            self.max_D,
            self.max_K,
...
def main(run, model):
    ...
    total_train_steps = ceil(7.85 * len(train_loader))
