...
def infer(model, loader, tta_level=0):
    ...
    @torch.compile(fullgraph=True)
    def _get_tta_logits_mirror(model, images_batch):
        # Simpler TTA: just original + mirror (2 views instead of 6)
        flipped = images_batch.flip(-1)
        combined = torch.cat([images_batch, flipped], dim=0)
        combined_logits = model(combined)
        batch_size = images_batch.shape[0]
        return 0.5 * combined_logits[:batch_size] + 0.5 * combined_logits[batch_size:]
    ...
    def tta(model, test_images) -> torch.Tensor:
        with torch.no_grad():
            ...
            # Use logit gap (top1 - top2) as confidence — cheaper than softmax
            top2_vals, _ = initial_logits.topk(2, dim=1)
            confidences = top2_vals[:, 0] - top2_vals[:, 1]
            ...
            for i in range(0, k_uncertain, tta_batch_size):
                ...
                logits_batch = _get_tta_logits_mirror(
                    model,
