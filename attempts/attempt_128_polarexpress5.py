...
# Attempt 128: Batched Polar Express Muon orthogonalizer.
# Replaces the Muon orthogonalization step with a compiled 5-stage Polar Express
# polynomial while keeping the baseline training recipe and selective TTA.
...
#############################################
#      Muon optimizer (batched PolarExp)    #
#############################################
...
def _zeropower_via_batched_polarexpress5(
    ...
    for original_shape, reshaped_D, reshaped_K, list_idx in filter_meta_data:
        g = gradients_4d[list_idx]
        out_dtypes.append(g.dtype)
        g_reshaped = g.reshape(reshaped_D, reshaped_K)
    ...
    X = torch.stack(grad_list).to(torch.bfloat16)
    ...
    # Polar Express normalizes so that the singular values are safely inside [-1, 1].
    ...
    for i, meta in enumerate(filter_meta_data):
        ...
        orthogonalized_g = X[i, :reshaped_D, :reshaped_K].reshape(original_shape)
        final_orthogonalized_grads_list[list_idx] = orthogonalized_g.to(out_dtypes[i])
...
class Muon(torch.optim.Optimizer):
    def __init__(
        ...
        self.max_K = ((max(1, self.max_K) + 15) // 16) * 16  # align for Tensor Cores
    ...
    def step(self):
        ...
        final_orthogonalized_grads = _zeropower_via_batched_polarexpress5(
            nesterov_grads,
            filter_meta_for_current_step,
            self.max_D,
            self.max_K,
