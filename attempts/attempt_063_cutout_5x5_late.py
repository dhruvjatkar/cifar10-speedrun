...
def main(run, model):
    ...
    # CutOut config: zero out a fixed 5x5 patch, applied in last 50% of training
    cutout_size = 5
    cutout_start_step = total_train_steps // 2
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Apply CutOut (zero patch) in last 50% of training
            if step >= cutout_start_step:
                cx = torch.randint(0, 32, (1,)).item()
                cy = torch.randint(0, 32, (1,)).item()
                x1 = max(0, cx - cutout_size // 2)
                y1 = max(0, cy - cutout_size // 2)
                x2 = min(32, x1 + cutout_size)
                y2 = min(32, y1 + cutout_size)
                inputs[:, :, y1:y2, x1:x2] = 0
