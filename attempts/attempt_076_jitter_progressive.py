...
def main(run, model):
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Progressive extra color jitter
            progress = step / max(1, total_train_steps)
            extra_brightness = 0.1399 * 2.0 * progress
            extra_contrast = 0.1308 * 2.0 * progress
            if extra_brightness > 0.01:
                inputs = batch_color_jitter(inputs, extra_brightness, extra_contrast)
