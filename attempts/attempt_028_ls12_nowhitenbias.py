...
def main(run, model):
    ...
    lr_factor_base = 1.0 / total_train_steps
    lr_factors_initial = [group["initial_lr"] for group in optimizer1.param_groups + optimizer2.param_groups]
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            # Always train whiten bias (eliminates compile variant)
            loss = forward_step(inputs, labels, True)
            loss.backward()

            # Update learning rates
            lr_factor = 1 - step * lr_factor_base
            for i, group in enumerate(optimizer1.param_groups + optimizer2.param_groups):
                group["lr"] = lr_factors_initial[i] * lr_factor
