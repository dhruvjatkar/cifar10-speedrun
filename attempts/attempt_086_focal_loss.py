...
def main(run, model):
    ...
    def forward_step(inputs, labels, whiten_bias_grad):
        ...
        n_classes = 10
        smooth = 0.09
        soft = smooth / n_classes
        targets = torch.full_like(log_probs, soft)
        targets.scatter_(1, labels.unsqueeze(1), 1.0 - smooth + soft)
        focal_weight = (1 - probs) ** 1.0  # gamma=1.0
        loss = -(targets * focal_weight * log_probs).sum()
        return loss
