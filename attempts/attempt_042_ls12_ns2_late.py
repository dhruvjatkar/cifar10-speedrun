...
def _zeropower_via_newtonschulz5(
    ...
    # AOL diagonal preconditioning (Turbo-Muon)
    # Compute Gram matrix row sums for Gershgorin-based preconditioner
    A0 = X @ X.transpose(1, 2)
    row_abs_sums = A0.abs().sum(dim=-1, keepdim=True)
    precond = torch.rsqrt(row_abs_sums + eps_stable)
    X = X * precond

    # Unroll the loop for better performance (3 NS iterations with preconditioning)
    A = X @ X.transpose(1, 2)
    B = b * A + c * (A @ A)
    X = a * X + B @ X
