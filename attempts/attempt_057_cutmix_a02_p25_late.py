...
def main(run, model):
    ...
    # CutMix config: standard CutMix, alpha=0.2, prob=25%, last 50% of training
    cutmix_alpha = 0.2
    cutmix_prob = 0.25
    ...
    def forward_step(inputs, labels, whiten_bias_grad):
        outputs = model(inputs, whiten_bias_grad=whiten_bias_grad)
        loss = F.cross_entropy(outputs, labels, label_smoothing=0.05, reduction="sum")
        return loss
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Apply standard CutMix (alpha=0.2, prob=25%) in last 50% of training
