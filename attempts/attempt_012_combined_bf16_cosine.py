...
def _zeropower_via_newtonschulz5(
    gradients_4d: list[torch.bfloat16],
...
class Muon(torch.optim.Optimizer):
    ...
    def _make_grad_hook(self, p, p_meta):
        """Create a hook that accumulates momentum as soon as grad is available."""
        def hook(param):
            ...
            if "momentum_buffer" not in state:
                state["momentum_buffer"] = torch.zeros_like(param.grad,
                    dtype=group.get("momentum_buffer_dtype", param.grad.dtype),
                    memory_format=torch.preserve_format)
            buf = state["momentum_buffer"]
            buf.mul_(group["momentum"])
            buf.add_(param.grad.to(buf.dtype))
            # Store pre-computed nesterov grad
            if group["nesterov"]:
                state["nesterov_grad"] = param.grad + group["momentum"] * buf
            else:
                state["nesterov_grad"] = buf.clone()
        return hook
    ...
    def setup_hooks(self):
        ...
        for p_meta in self.filter_params_meta:
            p = p_meta["param"]
            h = p.register_post_accumulate_grad_hook(self._make_grad_hook(p, p_meta))
    ...
    def step(self):
        ...
        # Gradients and momentum already accumulated by hooks
        ...
        for p_meta in self.filter_params_meta:
            p = p_meta["param"]
            state = self.state[p]
            if "nesterov_grad" in state:
                filter_params_with_grad.append(p)
                nesterov_grads.append(state["nesterov_grad"])
        ...
        # Clean up nesterov_grad entries so they don't persist across steps
        for p_meta in self.filter_params_meta:
            p = p_meta["param"]
            state = self.state[p]
            if "nesterov_grad" in state:
                del state["nesterov_grad"]
...
CIFAR_MEAN = torch.tensor((0.4914, 0.4822, 0.4465), dtype=torch.bfloat16)
CIFAR_STD = torch.tensor((0.247, 0.2435, 0.2616), dtype=torch.bfloat16)
...
def main(run, model):
    ...
    optimizer2.param_groups[0]["momentum_buffer_dtype"] = torch.bfloat16
    # Setup gradient hooks to overlap momentum accumulation with backward pass
    optimizer2.setup_hooks()
    ...
    # Precompute LR factors for cosine schedule
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Backward pass - gradient hooks will overlap momentum accumulation
            loss.backward()

            # Cosine LR schedule for whiten bias (decays to 0 over whiten_bias_train_steps)
            if step < whiten_bias_train_steps:
                lr_factor1 = 0.5 * (1 + cos(pi * step / whiten_bias_train_steps))
            else:
                lr_factor1 = 0.0
            # Cosine LR schedule for main params (decays to 0 over total_train_steps)
            lr_factor2 = 0.5 * (1 + cos(pi * step / total_train_steps))
    ...
    # Remove hooks before evaluation
    for h in optimizer2._hooks:
        h.remove()
    optimizer2._hooks = []
