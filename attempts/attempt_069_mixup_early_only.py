...
def main(run, model):
    ...
    # MixUp config: applied only in first 40% of training (mWh-inspired)
    mixup_alpha = 0.2
    mixup_end_step = int(0.4 * total_train_steps)

    # Compile the forward pass function for soft labels (MixUp)
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Apply MixUp only in first 40% of training
            if step < mixup_end_step:
