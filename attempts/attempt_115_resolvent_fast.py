...
# Attempt 115: Full-stack fast resolvent: AOL + Neumann + FP16 + compile.
# Combines all speedup techniques:
# (a) AOL diagonal preconditioner for better conditioning.
# (b) Neumann series for GEMM-only (no Cholesky, compilable).
# (c) FP16 throughout (skip .float() conversion, use tensor cores).
# (d) @torch.compile(fullgraph=True) on the resolvent function.
# Uses 4 resolvent terms, K=3 Neumann terms. Target: ~1.98s.
...
@torch.compile(fullgraph=True)
def _neumann_resolvent_apply(gram_scaled, X, shifts, weights, precond, neumann_terms):
    """AOL-preconditioned sum-of-resolvents via Neumann series.
    All GEMM, no Cholesky. FP16-safe, fully compilable.
    """
    B, D, K = X.shape
    ...
    result = result * precond.unsqueeze(-1)
    return result
...
def _zeropower_via_resolvents(
    ...
    # Stay in FP16 — skip .float() conversion for tensor core utilization
    X = torch.stack(grad_list)
    transposed = False
    if X.size(1) > X.size(2):
        X = X.transpose(1, 2)
    ...
    # AOL diagonal preconditioner
    row_norms = X.abs().sum(dim=-1).clamp_min(1e-4)  # (B, D)  — higher eps for FP16
    precond = row_norms.rsqrt()
    X_precond = X * precond.unsqueeze(-1)

    gram = X_precond @ X_precond.transpose(1, 2)
    eye = torch.eye(D, device=gram.device, dtype=gram.dtype).unsqueeze(0)

    gram_scale = gram.diagonal(dim1=-2, dim2=-1).mean(dim=-1).clamp_min(1e-4)
    ...
    orthogonalized = _neumann_resolvent_apply(
        gram_scaled, X_precond, shifts, weights, precond, neumann_terms
    )
