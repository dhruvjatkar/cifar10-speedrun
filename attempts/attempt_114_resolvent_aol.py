...
# Attempt 114: Resolvent Muon with AOL diagonal preconditioner + 4 terms.
# Based on attempt 110. Three speedups:
# (a) Direct solve (A+σI)Y=X instead of computing full inverse.
# (b) AOL preconditioner (Turbo-Muon) for better conditioning.
# (c) 4 Gauss-Legendre terms (better conditioning means fewer terms needed).
...
DEFAULT_RESOLVENT_TERMS = 4
DEFAULT_RESOLVENT_RIDGE = 1e-04

RESOLVENT_GAUSS_LEGENDRE_TABLES = {
    4: {
        "shifts": [
            0.005566997522987884,
            0.242613805021170400,
            4.121776994152252000,
            179.630042203303560000,
        ],
        "weights": [
            0.127865037554974470,
            0.462441798525286400,
            1.906081966295916000,
            22.968402092327060000,
        ],
    },
    6: {
        "shifts": [
...
def _zeropower_via_resolvents(
    ...
    # AOL diagonal preconditioner (Turbo-Muon):
    # P = diag(s_i) where s_i = (sum_j |G_ij|)^{-1/2}
    # This improves conditioning of the Gram matrix.
    row_norms = X.abs().sum(dim=-1).clamp_min(1e-8)  # (B, D)
    precond = row_norms.rsqrt()  # (B, D)
    X_precond = X * precond.unsqueeze(-1)  # (B, D, K)

    gram = X_precond @ X_precond.transpose(1, 2)
    ...
    # Build shifted matrices: (B, num_terms, D, D)
    shifted = gram_scaled.unsqueeze(1) + shifts.view(1, -1, 1, 1) * eye.unsqueeze(1)
    shifted = shifted.reshape(B * num_terms, D, D)

    # Direct solve: (A+σI) Y = X_precond
    chol, info = torch.linalg.cholesky_ex(shifted)
    if torch.any(info != 0):
        diag_mean = shifted.diagonal(dim1=-2, dim2=-1).mean(dim=-1).clamp_min(1e-6)
        eye_flat = torch.eye(D, device=shifted.device, dtype=shifted.dtype).unsqueeze(0)
        shifted = shifted + 1e-4 * diag_mean.view(-1, 1, 1) * eye_flat
        chol, info = torch.linalg.cholesky_ex(shifted)

    rhs = X_precond.unsqueeze(1).expand(B, num_terms, D, K).reshape(B * num_terms, D, K)
    solved = torch.cholesky_solve(rhs, chol)
    solved = solved.view(B, num_terms, D, K)

    # Weighted sum over quadrature terms
    # Result is P * G_precond^{-1/2} * P * X = P * (PGP)^{-1/2} * P * X
    # Which gives the preconditioned orthogonalization
    orthogonalized = (solved * weights.view(1, num_terms, 1, 1)).sum(dim=1)
    orthogonalized = orthogonalized / gram_scale.sqrt().view(-1, 1, 1)
    # Apply preconditioner to output
    orthogonalized = orthogonalized * precond.unsqueeze(-1)
