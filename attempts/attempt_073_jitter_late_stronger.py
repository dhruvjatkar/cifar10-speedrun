...
def main(run, model):
    ...
    # Stronger color jitter in last 50% of training (3x total)
    jitter_boost_start_step = total_train_steps // 2
    ...
    for epoch in range(ceil(total_train_steps / len(train_loader))):
        ...
        for inputs, labels in train_loader:
            ...
            # Apply extra color jitter in last 50% of training (2 extra = 3x total)
            if step >= jitter_boost_start_step:
                inputs = batch_color_jitter(inputs, 0.1399, 0.1308)
                inputs = batch_color_jitter(inputs, 0.1399, 0.1308)
