...
# Hybrid Muon with 2 NS iterations + row preconditioner.
# Key changes from baseline Muon:
# - Row preconditioner before polynomial iteration (cheap spectrum flattening)
# - Only 2 NS iterations instead of 3 (row preconditioner compensates)
...
def _wide_polar_fast(
    ...
    # 4) two fast polynomial polar steps (row preconditioner compensates for fewer iters)
    A = X @ X.transpose(1, 2)
