...
def infer(model, loader, tta_level=0):
    ...
    def _get_tta_logits(model, images_batch, pad):
        ...
        # Weighted average: 2x weight for original views (indices 0,3), 1x for crops
        # Views order: [orig, crop_tl, crop_br, orig_flip, crop_tl_flip, crop_br_flip]
        weights = torch.tensor([2.0, 1.0, 1.0, 2.0, 1.0, 1.0], device=reshaped_logits.device, dtype=reshaped_logits.dtype).view(-1, 1, 1)
        averaged_logits = (reshaped_logits * weights).sum(dim=0) / weights.sum()
